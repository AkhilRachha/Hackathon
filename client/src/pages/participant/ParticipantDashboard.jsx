...
                // FIX: Corrected API path to match the backend team routes mounting
                const response = await axios.get(`http://localhost:5000/api/teams/my-team/${userId}`);
                setMyTeam(response.data);

            } catch (err) {
                if (err.response && err.response.status === 404) {
                    // This correctly captures the 404 message ("You are not assigned to a team yet.")
...
    // --- Participant not allocated team UI (Triggers if setError was called) ---
    if (error && !myTeam) {
        return (
