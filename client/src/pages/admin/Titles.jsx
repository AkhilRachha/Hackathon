...
import { getDomainsAndCriteria, createQuestion, updateQuestion, deleteQuestion } from '@/api/questionApi'; // Use the correct API file
...
const Titles = () => {
    ...
    const fetchTitles = async () => {
        try {
            // Use the correct, imported API function
            const data = await getDomainsAndCriteria();
            if (Array.isArray(data)) {
                setDomains(data);
            } else {
                showToast({ title: "Warning", description: "Fetched data format is incorrect.", variant: 'destructive' });
            }
        } catch (error) {
            console.error("Error fetching data:", error);
            toast({ title: "Connection Error", description: "Failed to fetch data. Ensure the server is running.", variant: 'destructive' });
        } finally {
            setLoading(false);
        }
    };
                    ...
                    <div className="space-y-8">
                        {domains.length === 0 && (
                            <div className="text-center p-12 border-2 border-dashed rounded-xl bg-white">
                                ...
                                <p className="text-gray-500 mt-2">Click "Add Domain" to begin.</p>
                            </div>
                        )}
                                                ...
                                                <Card key={project._id || projectIndex} className="p-4 bg-white border">
                                                    <div className="space-y-4">
                                                        <div>
                                                            ...
                                                            {project._id && <p className="text-xs text-gray-500 mt-1">ID: <span className="font-mono">{project._id}</span></p>}
                                                        </div>
                                                        <div>
