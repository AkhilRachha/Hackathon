...
const Select = ({ value, onChange, className, children }) => <select value={value} onChange={onChange} className={`w-full border rounded-md p-2 ${className}`}>{children}</select>;
...
const TeamRegistration = ({ projects = [], participants = [], domains = [], onRegisterTeam, onCancel, isLoading }) => {
  ...
  useEffect(() => {
    if (domains.length > 0 && (!domains.includes(selectedDomain) || selectedDomain === '')) {
  ...
  const submitDisabled = isLoading || !selectedDomain || filteredProjects.length === 0 || form.watch('members')?.length === 0;
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        {/* {form.formState.errors.team_name && <FormMessage>{form.formState.errors.team_name.message}</FormMessage>} */}
                      </FormItem>
                    )}
                  />
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="space-y-4">
                        ...
                        {/* {form.formState.errors.q_id && <FormMessage>{form.formState.errors.q_id.message}</FormMessage>} */}
                      </FormItem>
                    )}
                  />
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        {/* {form.formState.errors.user_github_url && <FormMessage>{form.formState.errors.user_github_url.message}</FormMessage>} */}
                      </FormItem>
                    )}
                  />
                  ...
                  <div className="flex justify-end items-center pt-6 gap-4">
                    <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                      <Button type="button" onClick={onCancel} variant="ghost" className="h-10 text-sm text-gray-500 hover:text-gray-700 hover:bg-gray-100/50" disabled={isLoading}>
...
// --- Main App component to handle data fetching ---
const App = () => {
  ...
  useEffect(() => {
    ...
    const PARTICIPANTS_ENDPOINT = 'http://localhost:5000/api/users/participants/available';
    const PROJECTS_ENDPOINT = 'http://localhost:3000/api/questions/grouped'; 

    const fetchData = async () => {
        setIsLoading(true);

        // --- 1. Fetch Participants (Real Fetch) ---
        try {
            console.log("Fetching participants from:", PARTICIPANTS_ENDPOINT);
            const participantsResponse = await fetch(PARTICIPANTS_ENDPOINT);
            ...
            // Assuming participantsData is [{_id: "...", user_name: "..."}, ...]
            setParticipants(participantsData);
            console.log("Participants loaded successfully from backend.");
        } catch (error) {
            console.error("Error fetching participants:", error);
            // Fallback: use an empty array on error
            setParticipants([]); 
        }

        // --- 2. Fetch Projects and Domains (Real Fetch) ---
        try {
            ...
            // groupedData is expected to be an array of objects: 
            // [{ name: "Domain Name", projects: [...] }, ...]
            const groupedData = await projectsResponse.json(); 

            const allProjects = [];
            const domainNames = [];

            groupedData.forEach(domainGroup => {
                // Ensure domainGroup.name and domainGroup.projects exist
                if (domainGroup.name && domainGroup.projects) {
                    domainNames.push(domainGroup.name);
                    // Flatten projects and include the domain for filtering
                    domainGroup.projects.forEach(project => {
                        allProjects.push({ ...project, domain: domainGroup.name });
                    });
                }
            });
            ...
            console.log("Projects and Domains loaded successfully from backend.");
  ...
  const handleRegisterTeam = (data) => {
    const selectedProject = projects.find(p => p._id === data.q_id);
    const selectedMembers = participants.filter(p => data.members.includes(p._id));
    
    // Final payload ready for submission to your team registration API endpoint
    console.log("Team Registration Data (Ready for API submission):", {
        team_name: data.team_name,
        q_id: data.q_id, // Project ID
        members: data.members, // Array of Participant IDs
        user_github_url: data.user_github_url,
        project_domain: data.project_domain,
    });
    
    // In a real application, you would make a POST request to your team registration endpoint here.
    alert(`Team "${data.team_name}" registered for Project: "${selectedProject?.q_title}"! Check console for full data payload to send to your backend.`); 
  };
